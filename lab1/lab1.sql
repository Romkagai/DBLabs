...
upd. Ошибку понял, переделал выборку по "%Баранова%", а не по айдишникам

*/

SELECT DISTINCT firstTAB.name,
       street || ', дом ' || house_number || ', кв. ' || apartment as full_address,
       CASE WHEN area IS NULL THEN 'Нет' ELSE area END
FROM pd_orders
    JOIN pd_customers ON pd_orders.cust_id = pd_customers.id
    JOIN pd_employees firstTAB ON pd_orders.emp_id = firstTAB.id
    JOIN pd_employees secondTAB ON firstTAB.manager_id = secondTAB.id

WHERE
    (firstTAB.name ILIKE '%Баранова%' OR secondTAB.name ILIKE '%Баранова%') AND
